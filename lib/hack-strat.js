...
export async function main(ns) {
  ...
  while (true) {
    ...
    if (action !== currentAction || target !== currentTarget) {
      ...
      currentFleet = fleet;
      await deploy(ns, action, target, Boolean(options.terminal), true);
    } else if (fleet !== currentFleet) {
      currentFleet = fleet;
      await deploy(ns, action, target, Boolean(options.terminal), false);
    }
...
async function deploy(ns, action, target, terminal, fullRedeploy) {
  ...
  for (const host of servers) {
    if (fullRedeploy) {
      ns.killall(host);
    } else if (hasMatchingWorker(ns, host, script, target)) {
      continue;
    } else {
  ...
  log(ns, `hack-strat: ${mode} ${action} ${target} on ${launched}/${servers.length} server(s), ${totalThreads} thread(s).`, terminal);
}

function hasMatchingWorker(ns, host, script, target) {
  return ns.ps(host).some((process) =>
