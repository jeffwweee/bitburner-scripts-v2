...
export async function main(ns) {
  ...
  const homeReserve = hasFlag(ns.args, "--home-reserve")
    ? Math.max(0, Number(options["home-reserve"]) || DEFAULT_HOME_RESERVE)
    : reserveConfig.homeRamReserve;
  const useHome = !Boolean(options["no-home"]);
  let currentPlanKey = "";
  let currentTarget = "";
  let currentFleet = "";
  ...
  while (true) {
    ...
    const fleet = getFleetKey(ns, useHome, homeReserve);
    if (plan.key !== currentPlanKey || target !== currentTarget) {
      currentPlanKey = plan.key;
      currentTarget = target;
      currentFleet = fleet;
      await deploy(ns, plan, target, Boolean(options.terminal), true, useHome, homeReserve);
    } else if (fleet !== currentFleet) {
      currentFleet = fleet;
      await deploy(ns, plan, target, Boolean(options.terminal), false, useHome, homeReserve);
    } else if (hasDeploymentGaps(ns, plan, target, useHome, homeReserve)) {
      await deploy(ns, plan, target, Boolean(options.terminal), false, useHome, homeReserve);
    }
...
async function deploy(ns, plan, target, terminal, fullRedeploy, useHome, homeReserve) {
  const servers = getWorkerHosts(ns, useHome, homeReserve);
  ...
  for (const host of servers) {
    ...
    const allocations = getThreadAllocations(ns, host, plan, homeReserve);
...
function getThreadAllocations(ns, host, plan, homeReserve) {
  const growScriptRam = ns.getScriptRam(WORKERS.grow, host);
  const availableRam = getAvailableRamAfterClearingWorkers(ns, host, homeReserve);
...
function getAvailableRamAfterClearingWorkers(ns, host, homeReserve) {
  const reserve = host === "home" ? homeReserve : 0;
  return Math.max(0, ns.getServerMaxRam(host) - ns.getServerUsedRam(host) + getWorkerRamUsed(ns, host) - reserve);
}
...
function hasDeploymentGaps(ns, plan, target, useHome, homeReserve) {
  return getWorkerHosts(ns, useHome, homeReserve).some((host) => {
    const allocations = getThreadAllocations(ns, host, plan, homeReserve);
    if (getTotalAllocatedThreads(allocations) <= 0) return false;
    return !hasMatchingPlan(ns, host, allocations, target);
  });
}
...
function getWorkerHosts(ns, useHome, homeReserve) {
  const hosts = discoverServers(ns)
    .filter((host) => host !== "home")
    .filter((host) => ns.hasRootAccess(host))
    .filter((host) => ns.getServerMaxRam(host) > 0);

  if (useHome && ns.getServerMaxRam("home") > homeReserve) {
    hosts.push("home");
  }
...
function getFleetKey(ns, useHome, homeReserve) {
  return getWorkerHosts(ns, useHome, homeReserve)
...
function printHelp(ns) {
  ns.tprint("Usage: run lib/hack-strat.js [TARGET] [--target HOST] [--strategy current|prep] [--rank] [--top N] [--home-reserve GB] [--no-home] [--tail] [--terminal]");
