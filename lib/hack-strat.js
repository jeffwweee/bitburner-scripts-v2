...
async function deploy(ns, plan, target, terminal, fullRedeploy) {
  ...
  for (const host of servers) {
    const allocations = getThreadAllocations(ns, host, plan, fullRedeploy);
    if (getTotalAllocatedThreads(allocations) <= 0) continue;

    if (fullRedeploy) {
      killWorkerScripts(ns, host);
    } else if (hasMatchingPlan(ns, host, allocations, target)) {
      continue;
    } else {
      killWorkerScripts(ns, host);
    }
  ...
  log(ns, `hack-strat: ${mode} ${plan.name} ${target} on ${touched}/${servers.length} server(s): ${formatPlanTotals(totals)}.`, terminal);
}

function getThreadAllocations(ns, host, plan, fullRedeploy) {
  const growScriptRam = ns.getScriptRam(WORKERS.grow, host);
  const availableRam = fullRedeploy
...
function hasMatchingPlan(ns, host, allocations, target) {
  const expected = planSignature(allocations, target);
  return workerPlanSignature(ns, host) === expected;
}
