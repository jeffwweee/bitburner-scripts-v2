...
export async function main(ns) {
  ...
  if (!stock) {
    ns.print("stock-watch: ns.stock API is unavailable.");
    return;
  }

  const symbols = safeCall(() => stock.getSymbols(), []);
  if (!Array.isArray(symbols) || symbols.length === 0) {
    ns.print("stock-watch: no stock symbols visible. Buy WSE/TIX access first.");
    return;
  }
...
function printHelp(ns) {
  ns.tprint("Usage: run lib/stock-watch.js [--interval MS] [--window N] [--top N] [--once] [--tail] [--terminal]");
