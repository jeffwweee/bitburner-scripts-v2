...
export async function main(ns) {
  ...
  const fraction = parseFraction(options.fraction, DEFAULT_FRACTION);
...
async function ensureShare(ns, fraction, homeReserve, terminal) {
  ...
  if (desiredThreads <= 0) {
    if (currentThreads > 0) {
      ...
      log(ns, "share: stopped; no home RAM available above reserve.", terminal);
    }
    return;
  }
...
function getDesiredThreads(ns, fraction, homeReserve) {
  ...
  const workerRam = getCurrentThreads(ns) * scriptRam;
  const targetRam = Math.max(0, ns.getServerMaxRam("home") - homeReserve) * fraction;
  const reclaimableRam = Math.max(0, ns.getServerMaxRam("home") - ns.getServerUsedRam("home") + workerRam - homeReserve);
  return Math.floor(Math.min(targetRam, reclaimableRam) / scriptRam);
}
