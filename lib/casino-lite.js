...
export async function main(ns) {
  ...
  while (peakMoney < goal) {
    ...
    const outcome = await playHand(ns, dom.document, Boolean(options.basic), goal);
    if (outcome === "win") {
      ...
      const currentMoney = readOverviewMoney(dom.document);
      if (currentMoney >= goal) {
        log(ns, `casino: goal reached after an undetected final hand. Home money is ${formatMoney(currentMoney)}.`, terminal);
      } else {
        log(ns, `casino: hand ended in an unknown UI state at ${formatMoney(currentMoney)}. Stopping instead of throwing.`, terminal);
      }
      return;
    }
...
async function playHand(ns, document, basicStrategy, goal) {
  for (let i = 0; i < 30; i++) {
    if (readOverviewMoney(document) >= goal) return "goal";
...
async function getOutcome(ns, document) {
  ...
  if (await tryFind(ns, document, "//*[contains(text(), 'cheater') or contains(text(), 'kicked') or contains(text(), 'get out')]", 1)) return "kicked";
...
function printHelp(ns) {
  ns.tprint("Usage: run lib/casino-lite.js [options]");
