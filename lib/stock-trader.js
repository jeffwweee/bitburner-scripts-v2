...
export async function main(ns) {
  ...
  if (!stock) {
    fatal(ns, "stock-trader: ns.stock API is unavailable.");
    return;
  }

  const access = getAccess(stock);
  if (!access.wse || !access.tix) {
    fatal(ns, `stock-trader: WSE/TIX access required. WSE ${access.wse ? "yes" : "no"}, TIX ${access.tix ? "yes" : "no"}.`);
    return;
  }

  const symbols = safeCall(() => stock.getSymbols(), []);
  if (!Array.isArray(symbols) || symbols.length === 0) {
    fatal(ns, "stock-trader: no stock symbols visible. Buy WSE/TIX access first.");
    return;
  }

  const reserveConfig = loadReserveConfig(ns);
  const config = {
    budget: hasFlag(ns.args, "--budget") ? parseFraction(options.budget, DEFAULT_BUDGET) : reserveConfig.stocks.budget,
    reserve: hasFlag(ns.args, "--reserve") ? Math.max(0, Number(options.reserve) || DEFAULT_RESERVE) : reserveConfig.stocks.reserve,
    maxPosition: hasFlag(ns.args, "--max-position") ? parseFraction(options["max-position"], DEFAULT_MAX_POSITION) : reserveConfig.stocks.maxPosition,
  ...
  log(ns, `stock-trader: starting long-only trader. mode ${access.fourSigmaTix ? "4S forecast" : "trend fallback"}, budget ${Math.round(config.budget * 100)}%, reserve ${formatMoney(config.reserve)}, dry-run ${config.dryRun}.`, config.terminal);

  while (true) {
    updateHistory(stock, symbols, history);
    trade(ns, stock, symbols, history, config, access);
    await ns.sleep(config.interval);
  }
}

function trade(ns, stock, symbols, history, config, access) {
...
function getAccess(stock) {
  return {
    wse: callStockBoolean(stock, ["hasWseAccount", "hasWSEAccount"]),
    tix: callStockBoolean(stock, ["hasTixApiAccess", "hasTIXAPIAccess"]),
    fourSigmaData: safeCall(() => stock.has4SData(), false),
    fourSigmaTix: callStockBoolean(stock, ["has4SDataTixApi", "has4SDataTIXAPI"]),
  };
}

function callStockBoolean(stock, names) {
  for (const name of names) {
    if (typeof stock[name] !== "function") continue;
    return safeCall(() => Boolean(stock[name]()), false);
  }
