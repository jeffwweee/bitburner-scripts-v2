...
const SCRIPTS = {
  ...
  homeUpgrade: "lib/home-upgrade.js",
  factions: "lib/factions.js",
  share: "lib/share.js",
...
export async function main(ns) {
  ...
  do {
    ...
    if (!options["no-factions"]) {
      await runChild(ns, SCRIPTS.factions, [
        "--join",
        terminal ? "" : "--quiet",
      ].filter(Boolean));
    }

    if (!options["no-home-upgrade"]) {
      await runChild(ns, SCRIPTS.homeUpgrade, [
        "--budget",
        config.homeUpgradeBudget,
        "--reserve",
        config.moneyReserve,
        terminal ? "" : "--quiet",
      ].filter(Boolean));
...
function getConfig(ns, options, rawArgs) {
  ...
  const homeUpgradeBudget = hasFlag(rawArgs, "--home-upgrade-budget") ? Number(options["home-upgrade-budget"]) || reserveConfig.home.budget : reserveConfig.home.budget;
...
function printHelp(ns) {
  ...
  ns.tprint("  --home-upgrade-budget N Home RAM/core upgrade budget above reserve, default from reserve.json");
  ...
  ns.tprint("  --no-home-upgrade     Do not buy home RAM/core upgrades");
  ns.tprint("  --no-factions         Do not auto-join faction invitations");
