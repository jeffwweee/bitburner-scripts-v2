...
export async function main(ns) {
  ...
  do {
    ...
    if (!options["no-share"]) {
      ensureShare(ns, options["share-fraction"], options["home-reserve"], Boolean(options.tail), terminal);
    }

    if (options.stock && !options["no-stock"]) {
      ensureStockTrader(
        ns,
        options["stock-budget"],
        options["stock-reserve"],
        options["stock-max-position"],
        !Boolean(options["stock-live"]),
...
function printHelp(ns) {
  ...
  ns.tprint("  --stock-budget N      Stock budget fraction/percent above reserve, default 0.5");
  ns.tprint("  --stock-reserve MONEY Cash reserve for stock-trader, default $1b");
